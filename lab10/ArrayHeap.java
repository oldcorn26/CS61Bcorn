...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        int flag;
        if ((flag = parentIndex(index)) == 0 || flag > size) return;
        while (contents[flag].myPriority > contents[index].myPriority) {
            swap(flag, index);
            index = flag;
            if ((flag = parentIndex(index)) == 0 || flag > size) return;
        }
    ...
    private void sink(int index) {
        ...
        while (getNode(index) != null) {
            ...
            int mini = min(min(index, left), right);
            if (mini == left) {
                swap(index, left);
                index = left;
            } else if (mini == right) {
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i <= size; i++) {
            if (contents[i].myItem.equals(item)) {
                if (contents[i].myPriority > priority && i != 1) {
                    contents[i].myPriority = priority;
                    swim(i);
                } else if (contents[i].myPriority < priority && getNode(2 * i) != null) {
                    contents[i].myPriority = priority;
                    sink(i);
                }
                return;
    ...
    public void testChangePriority() {
        ...
        int i = 0;
        String[] expected = { "h", "b", "d", "e", "g", "i", "c", "a"};
        while (pq.size() > 1) {
            assertEquals(expected[i], pq.removeMin());
            i += 1;
        }
